 ...
 * Reason: 用户登录认证 <br/>
 * date: 2015年12月14日 下午2:05:13 <br/>
 * 
 */
@Controller(value = "H5LoginAuthenticationController")
@RequestMapping(value = "/H5/login/authentication", produces = "text/html;charset=UTF-8")
public class LoginAuthenticationController extends AbstractController {
    ...
    public String loginindex(HttpServletRequest request, HttpServletResponse response) {
        ...
        //TODO 改成发布的静态页面域名
        return "redirect:http://m.haaaaaa.com/views/app/user/login.html";
    }
    ...
    public String mobileLogin(HttpServletRequest request, HttpServletResponse response,
        ...
        try {
            ...
            if (super.getAuthenticationService().getMobileAuthenticationByNameCount(authentication) > 1) {
                returnData.setMsg("该用户信息错误");
            ...
            if (auth1.getPassword().equals(pwdSalt)) {
                //            if (true) {
                Authentication loginMobile = super.getAuthenticationService().getByUseridAndType(
                    auth1.getUser().getId(), "2");
                // 普通用户
                if (auth1.getUser().getUserInfo().getType().equals("1")) {
                    ...
                    // 2.15新增判断是否为激活账户
                    json.put("isActivate", auth1.getInvalid());
    ...
    @ResponseBody
    public String getArtistInfo(HttpServletRequest request, HttpServletResponse response,
    ...
    @ResponseBody
    public String interceptorupdateArtistInfo(HttpServletRequest request,
                                              HttpServletResponse response, ArtistInfo artistInfo) {
        response.addHeader("Access-Control-Allow-Origin", "*");
        ReturnData returnData = new ReturnData();
        // Long userId=Long.parseLong(request.getParameter("userId"));
    ...
    @ResponseBody
    public String interceptorupdateArtistInfoPhoto(HttpServletRequest request,
    ...
    @ResponseBody
    public String getTest(HttpServletRequest request, HttpServletResponse response, Long id)
                                                                                            throws Exception {
        response.addHeader("Access-Control-Allow-Origin", "*");
        ReturnData returnData = new ReturnData();
        JSONObject json = new JSONObject();
